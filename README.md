...
`<workspace>/src/<checkout-directory>` and will stop if that destination already
exists. The directory name is derived from the checkout, so renaming or moving
the checkout does not require changing the installer. For a clean reinstall,
...
dependencies instead. P7 uses the Python SDK's RBot TCP controller interface and
...
Launch a P7 RBot TCP arm:

```bash
ros2 launch realhand_ros2 arm.launch.py model:=P7 side:=left interface_type:=rbot interface_name:=192.168.10.21 world_frame:=urdf
```
...
`interface_name` is the RBot controller TCP host and `interface_type` must be
`rbot`. If `interface_name` and `interface_type` are omitted, the arm node uses
model-specific defaults: `can0`/`socketcan` for A7 Lite and
`192.168.10.21`/`rbot` for P7.
...
- A7 Lite Cartesian arm features require the Python SDK `kinetix` extra. P7 uses the Python SDK RBot TCP controller interface; set `interface_type:=rbot` and `interface_name` to the controller IP address.
